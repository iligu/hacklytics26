...
def compute_country_vector(group):
    ...
    # ── Out-of-pocket burden RISE ─────────────────────────────
    # Positive = population paying more themselves (donors abandoned them)
    ...
    # ── Measles GROWTH RATE ───────────────────────────────────
    # Are cases GROWING? Positive = disease getting worse while funding drops
    ...
    for r in results:
        ...
        if match["iso"] != "AFG":
            print(f"  {match['name']:25s}  fatigue={match['fatigue_score']:.3f}  "
                  f"measles_growth={match['measles_growth']:.2f}  "
                  f"gov_decay={match['gov_decay']:.2f}  "
